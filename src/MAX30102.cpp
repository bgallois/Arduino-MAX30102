...
bool MAX30102::begin() {
  ...
  setSampleAverage(DEFAULT_AVERAGE);

  return true;
}

void MAX30102::setSampleAverage(SampleAverage average) {
  uint8_t previous = readRegister(MAX30102_REG_FIFO_CONFIG);
  writeRegister(MAX30102_REG_FIFO_CONFIG, previous | average);
}
...
void MAX30102::setLedsPulseWidth(LEDPulseWidth ledPulseWidth) {
  ...
  writeRegister(MAX30102_REG_SPO2_CONFIGURATION,
...
void MAX30102::setSlot1(SlotSetting slotsetting) {
  ...
  writeRegister(MAX30102_REG_MULTI_LED_CTRL_1AND2,
                (previous & 0xf8) | slotsetting);
}
...
void MAX30102::setSlot3(SlotSetting slotsetting) {
  ...
  writeRegister(MAX30102_REG_MULTI_LED_CTRL_3AND4,
                (previous & 0xf8) | slotsetting);
}
...
void MAX30102::setRangeADC(ADCRange adcRange) {
  uint8_t previous = readRegister(MAX30102_REG_SPO2_CONFIGURATION);
  writeRegister(MAX30102_REG_SPO2_CONFIGURATION,
                (previous & 0x9f) | (adcRange << 5));
}

void MAX30102::update() { readFifoData(); }
...
void MAX30102::readFifoData() {
  ...
  toRead = (readRegister(MAX30102_REG_FIFO_WRITE_POINTER) -
            readRegister(MAX30102_REG_FIFO_READ_POINTER)) &
...
uint8_t MAX30102::getPartId() { return readRegister(0xff); }
